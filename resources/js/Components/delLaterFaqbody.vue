...
<template>
    <section class="mt-8 px-5 mb-[4rem] w-full font-body xl:px-32 pb-14">
        <div class="flex flex-col md:flex-row justify-between gap-[3rem]">
            <div class="flex flex-col justify-between max-w-3xl">
                <div class="text-center md:text-left">
                    <h1 class="text-3xl">Frequently Asked Questions</h1>
                    <p>
                        Find answers to common inquiries from our valued users
                    </p>
                </div>
                <div
                    class="hidden md:block relative -top-[1rem] right-[10.5rem]"
                >
    ...
    <div class="card w-full max-w-3xl">
        <Accordion :activeIndex="0" expandIcon="pi pi-angle-up" collapseIcon="pi pi-angle-down">
            <AccordionTab v-for="(item, index) in faqData" :key="index" :header="item.header">
                <p class="m-0">
                    {{ item.content }}</p>
            </AccordionTab>
